...
async fn main() {
  ...
  let ws_streams: Arc<Mutex<Vec<SplitSink<WebSocketStream<TcpStream>, Message>>>> = Arc::new(Mutex::new(Vec::new()));
  let thread_ws_streams = ws_streams.clone();
  tokio::spawn(async move {
    let listener = TcpListener::bind("localhost:8080").await.expect("Can't listen");
  ...
  if let Ok(mut lines) = read_lines(filename) {
    ...
    for line in lines {
      if let Ok(line) = line {
        ...
        socket.send_to(data.as_bytes(), target_addr).expect("failed to send message");
